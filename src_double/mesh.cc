...
// need exemtions here //
Mesh::Mesh(const int* nr, const double* x, const double *y){
    double dx, dy;
    int count=0;
    ...
    dx = (this->x[1]-this->x[0])/(double)nr[0], dy = (this->y[1]-this->y[0])/(double)nr[1];
    ...
    bdry_vals = (double*)calloc(order,sizeof(int));
...
void Mesh::deform(void (*map)(double*, double*, double*, double, int), double theta){
    double **v = vertices;
    int order = (nr[0]+1)*(nr[1]+1);
...
void Mesh::get_xy(double *xy, const int v) const {
    xy[0] = vertices[v][0];
    xy[1] = vertices[v][1];
}
//////
...
double Mesh::get_bound(const int v) const {
    double boundary;
...
void Mesh::get_arrays(double **vertices, int **cells, int **dof, int **is_bound, double **bdry_vals){
...
void annulus_seg_map(double *vertex, double *x, double *y, double theta, int s){
    double x_hat, y_hat;
